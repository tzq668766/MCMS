		...
		<@ms.contentPanel>
			...
			<@ms.form isvalidation=true name="appForm" style="width: 65%;" class="form-inline" action="${base}/manager/app/save.do">
            	...
            	<!--<div class="form-group ms-form-group">	
					<label for="appKeyword">启动手机端</label>
 
					<div class="ms-form-control" style="height:auto">
						<input type="checkbox" />是
					</div>
				</div-->
				<@ms.textarea name="appUrl"  label="域 名"  rows="4" value="${app.appUrl?default('')}" placeholder="使用回车换行可以输入多个域名地址，必须要加http://" validation={"maxlength":"100","required":"true","data-bv-notempty-message":"必填项目", "data-bv-stringlength-message":"长度在100个字符以内!"} />
				...
				<input type="hidden" name="appMobileStyle" value="${app.appMobileStyle?default('m')}"/>
				<#if SystemManager==true>
					<!--超级管理员对站点的管理-->
