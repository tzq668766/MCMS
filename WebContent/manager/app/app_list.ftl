...
<@ms.contentBody>
	...
	<@ms.contentPanel>
		<@ms.panelNav>
			<@ms.button class="btn btn-success addWebsite" toggle="modal" value="新增站点" icon="plus"/>
		</@ms.panelNav>
		<@ms.table head=['站点编号','标题','模板','域名','操作']>
       			<#if listApp?has_content>
	           		<#list listApp as listApp>
					<tr>
                        <td class="text-center basicId">${listApp.appId?c?default(0)}</td>
			            <td class="text-center" id="name">${listApp.appName?default(0)}</td>
			            <td class="text-center">${listApp.appStyle?default("暂无模板")}</td>
			            <td class="text-center">${listApp.appUrl?default(0)}</td>
